...
- [x] **N-6: SDK packaging and documentation** `session:sdk-packaging`
  Chat packages are not published in the release pipeline. No READMEs in any chat SDK directory. Blocks external consumption.

  **READMEs**
  - [x] `herald-chat-sdk-typescript/README.md` — WS client wrapper, frame types, E2EE integration
  - [x] `herald-chat-core/README.md` — ChatCore API, stores, optimistic sends, liveness, scroll state
  - [x] `herald-chat-react/README.md` — Provider, hooks, render-prop components, useSyncExternalStore pattern
  - [x] `contract-tests/README.md` — How to run, spec format, adding new SDKs

  **Release pipeline**
  - [x] Add `herald-chat-sdk` to `publish-npm` job — scope to `@skeptik-io/herald-chat-sdk`, build + publish
