...
- **Send path — use persist-first, not optimistic-first.** The chat SDK defaults to publishing over the WebSocket, which makes Herald's ack the commit point and exposes senders to "message vanishes on refresh" when the WS publish fails mid-flight (the classic media-attachment orphan bug). Instead, pass a `writer` to `ChatCore` / `HeraldChatProvider` that routes sends through your own API — your DB commit becomes the commit point, Herald becomes the fanout pipe, and sender durability is structurally guaranteed. See [ARCHITECTURE.md §8 "Write Path: Persist-First vs Optimistic-First"](ARCHITECTURE.md#write-path-persist-first-vs-optimistic-first) and the [herald-chat README](herald-chat-core/README.md#persist-first-writes-recommended) for the full pattern.
