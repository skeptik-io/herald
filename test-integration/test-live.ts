...
`;
  ...
  try {
    ...
    await test("mt: free-tier tenant hits stream limit", async () => {
      // Create a free-tier tenant (max_streams = 10)
      await mtAdmin.tenants.create({
        id: "free-corp",
        name: "Free Corp",
        jwt_secret: JWT_SECRET,
      ...
      // Get an API token for this tenant
      const tokenResult = await mtAdmin.tenants.createToken("free-corp");
      const freeAdmin = new HeraldAdmin({
        url: `http://127.0.0.1:${HTTP_PORT2}`,
        token: tokenResult.token,
      });

      // Create 10 streams (the limit for free plan)
      for (let i = 0; i < 10; i++) {
        await freeAdmin.streams.create(`free-stream-${i}`, `Free Stream ${i}`);
      }

      // 11th stream should be rejected
      let hitLimit = false;
      try {
        await freeAdmin.streams.create("free-stream-overflow", "Over Limit");
      } catch (e: any) {
        hitLimit = true;
      }
      assert(hitLimit, "free-tier tenant should hit stream limit at 10");
    });

    await test("mt: pro-tier tenant does not hit stream limit at 10", async () => {
      // Create a pro-tier tenant (max_streams = 1000)
      await mtAdmin.tenants.create({
        id: "pro-corp",
        name: "Pro Corp",
        jwt_secret: JWT_SECRET,
      ...
      const tokenResult = await mtAdmin.tenants.createToken("pro-corp");
      const proAdmin = new HeraldAdmin({
        url: `http://127.0.0.1:${HTTP_PORT2}`,
        token: tokenResult.token,
      });

      // Create 11 streams — should succeed for pro plan
      for (let i = 0; i < 11; i++) {
        await proAdmin.streams.create(`pro-stream-${i}`, `Pro Stream ${i}`);
      }

      // All 11 should exist
      const streams = await proAdmin.streams.list();
      assert(streams.length >= 11, `pro-tier should have at least 11 streams, got ${streams.length}`);
    });

    await test("mt: free-tier tenant hits connection limit", async () => {
      // free plan: max_connections = 50. Connect 55 to verify cap.
      const tenantId = "free-corp"; // already created with "free" plan + JWT_SECRET
      const clients: HeraldClient[] = [];
      let rejected = false;

      try {
        for (let i = 0; i < 55; i++) {
          const token = jwt.sign(
            { sub: `user-${i}`, tenant: tenantId, streams: ["free-stream-0"], iss: "test" },
            JWT_SECRET,
            { expiresIn: "1h" },
          );
          const client = new HeraldClient({
            url: `ws://127.0.0.1:${WS_PORT2}/ws`,
            token,
          });
          try {
            await client.connect();
        ...
        assert(rejected, `free-tier should reject connections after 50, connected ${clients.length}`);
        assert(clients.length <= 50, `should have connected at most 50, got ${clients.length}`);
      } finally {
    ...
    await test("mt: rate limit varies by plan", async () => {
      // Free plan: api_rate_limit = 60 req/60s
      // Pro plan: api_rate_limit = 1000 req/60s
      // Blast 80 rapid requests at each tenant's API and compare

      const freeToken = (await mtAdmin.tenants.createToken("free-corp")).token;
      const proToken = (await mtAdmin.tenants.createToken("pro-corp")).token;

      async function blastRequests(token: string, count: number): Promise<number> {
        let rejections = 0;
        const promises = [];
        for (let i = 0; i < count; i++) {
          promises.push(
            fetch(`http://127.0.0.1:${HTTP_PORT2}/streams`, {
              headers: { Authorization: `Bearer ${token}` },
            }).then((r) => {
              if (r.status === 429) rejections++;
            })
          );
        }
      ...
      const freeRejections = await blastRequests(freeToken, 80);
      const proRejections = await blastRequests(proToken, 80);

      // Free (limit 60) should get some 429s at 80 requests
      assert(freeRejections > 0, `free-tier should get 429s at 80 req, got ${freeRejections} rejections`);
      // Pro (limit 1000) should get none at 80 requests
      assert(proRejections === 0, `pro-tier should not get 429s at 80 req, got ${proRejections} rejections`);
    });
